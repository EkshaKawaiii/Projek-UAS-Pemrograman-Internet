...
// Get the transaction number from URL
$no_trx = $_GET['no_trx'];

// Fetch the rental data for the given transaction number
$query = mysqli_query($koneksi, "SELECT * FROM tbl_rental_eksha WHERE no_trx_eksha='$no_trx'");
$data = mysqli_fetch_array($query);

// If the form is submitted, update the rental data
if (isset($_POST['submit'])) {
    ...
    $harga = $_POST['harga'];
    $lama = $_POST['lama'];
    $total_bayar = $harga * $lama;  // Calculate total payment

    // Update rental data in the database
    mysqli_query($koneksi, "UPDATE tbl_rental_eksha SET 
        tgl_rental_eksha='$tgl_rental', 
        jam_rental_eksha='$jam_rental', 
        harga_eksha='$harga', 
        lama_eksha='$lama', 
        total_bayar_eksha='$total_bayar' 
        WHERE no_trx_eksha='$no_trx'");

    // Redirect to rental list page after successful update
